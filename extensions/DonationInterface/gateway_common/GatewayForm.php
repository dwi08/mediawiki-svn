...
class GatewayForm extends UnlistedSpecialPage {
	 ...
	 */
	protected function fnValidateForm( &$data, &$error ) {
		...
		$msg = array(
			'amount' => wfMsg( $gateway_identifier . '_gateway-error-msg-amount' ),
			'emailAdd' => wfMsg( $gateway_identifier . '_gateway-error-msg-emailAdd' ),
			'fname' => wfMsg( $gateway_identifier . '_gateway-error-msg-fname' ),
			'lname' => wfMsg( $gateway_identifier . '_gateway-error-msg-lname' ),
			'street' => wfMsg( $gateway_identifier . '_gateway-error-msg-street' ),
			'city' => wfMsg( $gateway_identifier . '_gateway-error-msg-city' ),
			'state' => wfMsg( $gateway_identifier . '_gateway-error-msg-state' ),
			'zip' => wfMsg( $gateway_identifier . '_gateway-error-msg-zip' ),
			'card_num' => wfMsg( $gateway_identifier . '_gateway-error-msg-card_num' ),
			'expiration' => wfMsg( $gateway_identifier . '_gateway-error-msg-expiration' ),
			'cvv' => wfMsg( $gateway_identifier . '_gateway-error-msg-cvv' ),
		);
		...
		foreach ( $data as $key => $value ) {
			if ( $value == '' || ($key == 'state' && $value == 'YY' ) ) {
				...
				if ( isset( $msg[$key] ) ) {
					$error[$key] = "**" . wfMsg( $gateway_identifier . '_gateway-error-msg', $msg[$key] ) . "**<br />";
					$error_result = '1';
				}
			}
		}
		...
		if ( !preg_match( '/^\d+(\.(\d+)?)?$/', $data['amount'] ) ||
			...
			$error['invalidamount'] = wfMsg( $gateway_identifier . '_gateway-error-msg-invalid-amount' );
			$error_result = '1';
		}
		...
		if ( !$isEmail ) {
			$error['emailAdd'] = wfMsg( $gateway_identifier . '_gateway-error-msg-email' );
			$error_result = '1';
		}
