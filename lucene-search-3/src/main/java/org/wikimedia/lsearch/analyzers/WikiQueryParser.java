...
public class WikiQueryParser {
	...
	protected enum ExpandedType {
		WORD, WILDCARD, FUZZY, PHRASE
	};
	...
	/**
	 * sometimes the fieldsubquery takes the bool modifier, to retrieve it, use
	 * this variable,
	 ...
	 * REWRITE - rewrite (help:searchterm) => (+namespace:12
	 * contents:searchterm)
	 */
	public enum NamespacePolicy {
		LEAVE, IGNORE, REWRITE
	};
	...
	protected Pattern urlPattern = Pattern
			.compile("(\\w+:{0,1}\\w*@)?(\\S+)(:[0-9]+)?(\\/|\\/([\\w#!:.?+=&%@!\\-\\/]))?");
	...
	static class ParsedWords {
		...
		void add(String original, ArrayList<String> words,
				ArrayList<Float> boosts, ExpandedType type) {
		...
		void add(String original, ArrayList<String> words, float boost,
				ExpandedType type) {
			int pos = this.words.size();
			WordsDesc wd = new WordsDesc(original, type, pos);
			for (int i = 0; i < words.size(); i++) {
		...
		ArrayList<String> extractFirst() {
			...
			for (WordsDesc d : words) {
				if (d.type == ExpandedType.WORD
						|| d.type == ExpandedType.PHRASE)
					ret.add(d.first());
			}
			return ret;
		}
		...
		/**
		 * Get ParsedWords of first words, or whole ParsedWords if
		 * wildcard/fuzzy
		 */
	...
	protected void initNamespaces() {
		...
		for (Entry<String, NamespaceFilter> prefix : namespaceFilters
				.entrySet()) {
			namespaceQueries.put(prefix.getKey(),
					generateRewrite(prefix.getValue()));
	...
	public WikiQueryParser(String field, Analyzer analyzer,
			FieldBuilder.BuilderSet builder, Collection<String> stopWords) {
		this(field, (NamespaceFilter) null, analyzer, builder,
				NamespacePolicy.LEAVE, stopWords);
	}
	 ...
	 * @param nsPolicy
	 */
	public WikiQueryParser(String field, String namespace, Analyzer analyzer,
			FieldBuilder.BuilderSet builder, NamespacePolicy nsPolicy,
			Collection<String> stopWords) {
		this(field, new NamespaceFilter(namespace), analyzer, builder,
				nsPolicy, stopWords);
	}
	...
	public WikiQueryParser(String field, NamespaceFilter nsfilter,
			Analyzer analyzer, FieldBuilder.BuilderSet builder,
			NamespacePolicy nsPolicy, Collection<String> stopWords) {
		defaultField = field;
		...
		if (nsfilter != null) {
			...
			if (namespaceRewriteQuery != null
					&& namespacePrefixes.containsKey(nsfilter))
	...
	public static Query generateRewrite(NamespaceFilter nsfilter) {
		if (nsfilter.cardinality() == 0)
			...
			return new TermQuery(new Term("namespace",
					Integer.toString(nsfilter.getNamespace())));
		...
		for (int i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i + 1)) {
			...
			bq.add(new TermQuery(new Term("redirect_namespace", Integer
					.toString(i))), BooleanClause.Occur.MUST_NOT);
		}
		return bq;
	}
	...
	public static Query generateRedirectRewrite(NamespaceFilter nsfilter) {
		if (nsfilter.cardinality() == 0)
			...
			return new TermQuery(new Term("redirect_namespace",
					Integer.toString(nsfilter.getNamespace())));
		...
		for (int i = bs.nextSetBit(0); i >= 0; i = bs.nextSetBit(i + 1)) {
			bq.add(new TermQuery(new Term("redirect_namespace", Integer
					.toString(i))), BooleanClause.Occur.SHOULD);
		}
		return bq;
	}
	...
	public HashSet<NamespaceFilter> getFieldNamespaces(String queryText) {
		...
		for (String field : fields) {
			...
			if (namespaceFilters.containsKey(field))
				...
				ret.add(new NamespaceFilter(field.substring(1,
						field.length() - 1)));
	...
	public ArrayList<Token> tokenizeForSpellCheck(String queryText) {
		...
		this.analyzer = Analyzers.getReusableAnalyzer(filters,
				new TokenizerOptions.SpellCheckSearch());
				...
				if (tokenType == TokenType.WORD
					...
					for (Token t : tokens) {
						if (t.getPositionIncrement() > 0) {
							ret.add(new Token(t.termText(), start
									+ t.startOffset(), start + t.endOffset(),
									type));
						}
					}
				}
	...
	private Query getNamespaceQuery(String fieldName) {
		...
		if ((q = namespaceQueries.get(fieldName)) != null) {
			...
			return generateRewrite(new NamespaceFilter(fieldName.substring(1,
					fieldName.length() - 1)));
		} else
			return null;
	}
	...
	private NamespaceFilter getNamespaceFilter(String fieldName) {
		if (fieldName == null)
			...
			return new NamespaceFilter(fieldName.substring(1,
					fieldName.length() - 1));
		else
			return defaultNamespaceFilter;
	}
	...
	/**
	 * Fetch token into <code>buffer</code> starting from current position (
	 * <code>cur</code>)
	 * 
	...
	private TokenType fetchToken(boolean termOnly) {
		...
		for (length = 0; cur < queryLength; cur++) {
			...
			// pluses and minuses, underscores can be within words (to prevent
			// to be missinterpeted), *,? are for wildcard queries
			if (isTermChar(ch)) {
		...
		// check for keywords
		if (length == 3 && buffer[0] == 'A' && buffer[1] == 'N'
				&& buffer[2] == 'D')
		...
		for (lookup = cur + 1; lookup < queryLength; lookup++) {
			...
			if (ch == ' ')
				...
				if (f.equals(namespaceAllKeyword) || fieldOperators.contains(f)
	...
	/**
	 * Analyze a string, and return tokens (doesn't use any of the object
	 * storage attributes)
	 */
	...
	private Term makeTerm(String t) {
		...
		if (currentField == null)
			return new Term(defaultField, builder.isExactCase() ? t
					: t.toLowerCase());
		else if (defaultField.equals("contents") && isInTitle)
			return new Term("title", builder.isExactCase() ? t
					: t.toLowerCase());
		else if (currentField.equals("incategory")) {
			String norm = t.replace("_", " "); // bug 10822
			return new Term("category", builder.isExactCase() ? norm
					: norm.toLowerCase());
			...
			return new Term(field, t);
				...
				&& (namespacePolicy == NamespacePolicy.IGNORE || namespacePolicy == NamespacePolicy.REWRITE))
	...
	private Query parsePhrase() {
		...
		// PositionalMultiQuery query = new PositionalMultiQuery(new
		// PositionalOptions.PhraseQueryFallback());
		MultiPhraseQuery query = new MultiPhraseQuery();
		for (; cur < queryLength; cur++) {
			length = 0;
			// fetch next word
			while (cur < queryLength && isTermChar(text[cur])
			...
			if (length > 0) {
				...
				if (bufferIsWildCard()) {
					Term term = makeTerm();
					Term[] terms = wildcards.makeTerms(term.text(),
							term.field());
					if (terms != null) {
						query.add(terms);
						ArrayList<String> words = wildcards.getWords(term
								.text());
						parsedWords.add(term.text(), words, 1f,
								ExpandedType.WILDCARD);
						added = true;
					}
				}
				if (bufferIsFuzzy()) {
					...
					Term[] terms = fuzzy.makeTerms(term.text(), term.field(),
							nsf);
					if (terms != null) {
						// query.add(terms,fuzzy.getBoosts(term.text(),nsf,terms));
						query.add(terms);
						ArrayList<String> words = fuzzy.getWords(term.text(),
								nsf);
						parsedWords.add(term.text(), words,
								fuzzy.getBoosts(term.text(), nsf, words),
								ExpandedType.FUZZY);
						added = true;
					}
				}
				if (!added) {
					...
					for (Token token : tokens) {
						if (token.getPositionIncrement() > 0) { // ignore
																// aliases and
																// stemmed words
	...
	private final boolean needsRewrite() {
		return namespaceRewriteQuery != null
	 ...
	 * @param level
	 *            - level of recurstion
	 * @param returnOnFieldDef
	 *            - if this is a nested field rewrite call
	 * @return
	 */
	private Query parseClause(int level, boolean returnOnFieldDef,
			String topFieldName) {
		// the whole query
		Query query = null;
		...
		Query fieldsubquery = null; // e.g. 'all:something else' will be parsed
									// 'something else'
			...
			if (Character.isLetterOrDigit(c) || c == '.' || c == '['
				...
				case FIELD:
					...
					if (returnOnFieldDef) {
						...
						if (!newfield.equals("incategory")
								&& !newfield.equals(topFieldName)) {
							backToken();
							cur--;
							break mainloop;
						}
					...
					if (currentField == null || definedExplicitField) {
						...
						if ((defaultNamespaceName != null && currentField
								.equals(defaultNamespaceName))
								|| currentField.equals(defaultField)) {
							currentField = null;
							break; // repeated definition of field, ignore
						}
						definedExplicitField = true;

						fieldQuery = getNamespaceQuery(currentField); // depending
																		// on
																		// policy
																		// rewrite
																		// this
																		// field
						if (fieldQuery != null) {
							// save field, we will need it to be set to null to
							// fetch categories
							String myfield = currentField;
							currentField = null;
							// fetch the clause until the next field
							fieldsubquery = parseClause(level + 1, true,
									myfield);
				...
				case WORD:
					if (fieldQuery != null) {
						backToken();
						String myfield = (topFieldName != null) ? topFieldName
								: (currentField != null) ? currentField
										: (defaultNamespaceName != null) ? defaultNamespaceName
												: defaultField;
						fieldsubquery = parseClause(level + 1, true, myfield);
					} else {
						analyzeBuffer();
						subquery = makeQueryFromTokens(explicitOccur != null ? explicitOccur
								: occur);
					}
					break;
				case AND:
	 ...
	 * the only allowed patterns are *q and q* and not other combinations like
	 * *q* or q*r
	 */
	...
	private Query makeQueryFromTokens(BooleanClause.Occur toplevelOccur) {
		...
		// check for wildcard seaches, they are also not analyzed/stemmed, only
		// for titles
		// wildcard signs are allowed only at the end of the word, minimum one
		// letter word
		...
		if (length > 1 && fuzzy != null && bufferIsFuzzy()) {
			...
			if (ret != null) {
				...
				parsedWords.add(term.text(), words,
						fuzzy.getBoosts(termText, nsf, words),
						ExpandedType.FUZZY);
	...
	public String extractPrefixFilter(String queryText) {
		...
		while (start < queryText.length()) {
			...
			if (inx >= 0 && inx < end) {
				String[] prefixes = queryText.substring(
						inx + "prefix:".length()).split("\\|");
	...
	/**
	 * Like string.indexOf but return end of string instead of -1 when needle is
	 * not found
	 */
	...
	public static class ParsingOptions {
		...
		/**
		 * only parse the main query (on contents and title) without relevance
		 * stuff
		 */
	...
	public Query parse(String queryText, ParsingOptions options) {
		...
		// additional queries
		// Query related = new
		// LogTransformScore(makeRelatedRelevance(words,ADD_RELATED_BOOST));
		...
		Query redirectMatch = makeAlttitleForRedirectsMulti(
				makeFirstAndSingular(words), 20, 1f);
		...
		for (Entry<String, String> e : rawFields.entrySet()) {
			...
			// find target field in the index, e.g. ondiscussionpage ->
			// ThreadPage
			String targetField = keywordFieldMapping.get(field);
			if (targetField != null)
				full.add(new TermQuery(new Term(targetField, e.getValue())),
						Occur.MUST);
		}
		...
		if (age != AgeScaling.NONE) {
			switch (age) {
			case STRONG:
				scale = new ArticleScaling.StepScale(0.3f, 1);
				break;
			case MEDIUM:
				scale = new ArticleScaling.StepScale(0.6f, 1);
				break;
			case WEAK:
				scale = new ArticleScaling.StepScale(0.9f, 1);
				break;
			default:
				throw new RuntimeException("Unsupported age scaling " + age);
			}

		}

		// additional rank
		AggregateInfo rank = iid.useAdditionalRank() ? new AggregateInfoImpl()
				: null;
		...
		if (filters.hasSingular()) {
			...
			for (WordsDesc wd : ret.words) {
				...
				if (sw != null && !w.equals(sw)) {
					wd.add(new WordBoost(sw, wd.firstWordBoost().boost
							* SINGULAR_WORD_BOOST));
				}
			}
		}
	...
	protected Query makeTitlePart(String queryText) {
		...
		defaultBoost = (qs != null) ? TITLE_BOOST : TITLE_BOOST
				+ STEM_TITLE_BOOST;
	...
	protected Query makeMainPhraseWithRelevance(ParsedWords words,
			ParsedWords noStopWords) {
		Query main = null;
		String field = fields.contents(); // put to begin() for performance

		// all words as entered into the query
		Query phrase = makePositionalMulti(noStopWords, field,
				new PositionalOptions.Sloppy(), MAINPHRASE_SLOP, 1);

		Query sections = makeSectionsQuery(noStopWords, SECTIONS_BOOST);
		// wordnet synonyms
		ArrayList<ArrayList<String>> wordnet = WordNet.replaceOne(
				words.extractFirst(), iid.getLangCode());
			...
			if (wordnet != null) {
				for (ArrayList<String> wnwords : wordnet) {
					if (!allStopWords(wnwords))
						combined.add(
								makePositional(wnwords, field,
										new PositionalOptions.Sloppy(),
										MAINPHRASE_SLOP, 1), Occur.SHOULD);
				}
			}
			// urls
			if (urls.size() > 0) {
				for (ArrayList<Term> terms : urls) {
					combined.add(
							makePositional(extractTermText(terms),
									extractField(terms),
									new PositionalOptions.Sloppy(), 0, 1),
							Occur.SHOULD);
				}
			}
		}
		...
		if (wordnet != null)
			for (ArrayList<String> wnwords : wordnet)
				if (!allStopWords(wnwords))
					altAdd.add(makeAlttitleRelevance(wnwords,
							RELEVANCE_ALTTITLE_BOOST));
		...
		if (wordnet != null)
			for (ArrayList<String> wnwords : wordnet)
				if (!allStopWords(wnwords))
					relAdd.add(makeRelatedRelevance(wnwords,
							RELEVANCE_RELATED_BOOST));
	...
	/**
	 * Combine one main query with a number of other queries into a boolean
	 * query
	 */
	...
	private void extractAndSimplify(BooleanQuery bq, HashMap<Query, Float> map,
			float parentBoost) {
		for (BooleanClause cl : bq.getClauses()) {
			Query q = cl.getQuery();
			if (q instanceof BooleanQuery)
				extractAndSimplify((BooleanQuery) q, map,
						parentBoost * bq.getBoost());
			else {
	...
	protected PositionalQuery makePositional(ArrayList<String> words,
			String field, PositionalOptions options, int slop, float boost) {
	...
	protected Query makePositionalMulti(ParsedWords parsed, String field,
			PositionalOptions options, int slop, float boost) {
		PositionalMultiQuery mq = new PositionalMultiQuery(options);
		for (WordsDesc wd : parsed.words) {
			mq.addWithBoost(wd.getTerms(field), wd.getPosition(),
					wd.getBoosts());
		}
		mq.setSlop(slop);
		mq.setBoost(boost);
	...
	protected Query makeAnchoredQuery(ArrayList<String> words, String field,
			PositionalOptions options, PositionalOptions whole,
			PositionalOptions wholeSloppy, float boost, int slop) {
		BooleanQuery bq = new BooleanQuery(true);
		if (words.size() == 1) {
			...
			while (i < words.size()) {
				...
				if (phrase.size() > 1)
					bq.add(makePositional(phrase, field, options, 0,
							phrase.size()), Occur.SHOULD);
			}
		}
	...
	protected Query makeAnchoredQueryMulti(ParsedWords words, String field,
			PositionalOptions options, PositionalOptions whole, int slopWhole,
			float boost) {
		BooleanQuery bq = new BooleanQuery(true);
		...
		if (words.size() >= 2) {
			// add single words
			for (int i = 0; i < words.size(); i++) {
				if (!stopWords.contains(words.firstAt(i))) // skip single stop
															// words
					bq.add(makePositionalMulti(words.cloneSingleWord(i), field,
							options, 0, 1), Occur.SHOULD);
			}
			...
			if (words.size() >= 3) {
				for (int i = 0; i < words.size() - 1;) {
					...
					bq.add(makePositionalMulti(words.cloneRange(i1, i2), field,
							options, 10, 2), Occur.SHOULD);
					i = i2;
				}
			}
		}
		// add the whole-only query
		if (whole != null)
			bq.add(makePositionalMulti(words, field, whole, slopWhole, 1),
					Occur.SHOULD);
	...
	protected Query makeSectionsQuery(ParsedWords words, float boost) {
		return makeAnchoredQueryMulti(words, fields.sections(),
				new PositionalOptions.Sections(),
				new PositionalOptions.SectionsWhole(), 0, boost);
	}
	...
	protected Query makeAlttitleRelevance(ParsedWords words, float boost) {
		return makeAnchoredQueryMulti(words, fields.alttitle(),
				new PositionalOptions.Alttitle(),
				new PositionalOptions.AlttitleWholeSloppy(), 20, boost);
	}
	...
	protected Query makeRelatedRelevance(ParsedWords words, float boost) {
		return makeAnchoredQueryMulti(words, fields.related(),
				new PositionalOptions.Related(),
				new PositionalOptions.RelatedWhole(), 0, boost);
	}

	/** Relevance metrics based on rank (of titles and redirects) */
	protected Query makeAlttitleRelevance(ArrayList<String> words, float boost) {
		return makeAnchoredQuery(words, fields.alttitle(),
				new PositionalOptions.Alttitle(),
				new PositionalOptions.AlttitleWhole(),
				new PositionalOptions.AlttitleWholeSloppy(), boost, 20);
	}

	/** Make relevance metrics based on context via related articles */
	protected Query makeRelatedRelevance(ArrayList<String> words, float boost) {
		return makeAnchoredQuery(words, fields.related(),
				new PositionalOptions.Related(), null, null, boost, 0);
	}

	/**
	 * Additional query to match words in redirects that are not in title or
	 * article
	 */
	protected Query makeAlttitleForRedirects(ArrayList<String> words, int slop,
			float boost) {
		return makePositional(words, fields.alttitle(),
				new PositionalOptions.RedirectMatch(), slop, boost);
	}

	protected Query makeAlttitleForRedirectsMulti(ParsedWords words, int slop,
			float boost) {
		return makePositionalMulti(words, fields.alttitle(),
				new PositionalOptions.RedirectMatch(), slop, boost);
	}
	...
	public Query makeAlttitleForTitles(List<String> words) {
		...
		PositionalQuery exact = new PositionalQuery(
				new PositionalOptions.AlttitleExact());
		PositionalQuery sloppy = new PositionalQuery(
				new PositionalOptions.AlttitleSloppy());
	...
	public Query parseForTitles(String queryText) {
		...
		// match whole titles
		Query redirectsMulti = makeAlttitleForRedirectsMulti(
				makeFirstAndSingular(words), 20, 1f);
		if (redirectsMulti != null)
			full.add(redirectsMulti, Occur.SHOULD);

		ArticleNamespaceScaling nsScale = iid.getNamespaceScaling();
		return new ArticleQueryWrap(full, new ArticleInfoImpl(), null, null,
				nsScale);
