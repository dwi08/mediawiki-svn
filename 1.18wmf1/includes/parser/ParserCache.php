...
class ParserCache {
	...
	public function save( $parserOutput, $article, $popts ) {
		...
		if( $expire > 0 ) {
			...
			// HACK put host name in the parser cache comment --Roan
			$hostname = wfHostname();
			...
			// HACK 2: check for occurrences of bug 31576
			$templates = $parserOutput->getTemplates();
			if ( isset( $templates[NS_TEMPLATE] ) ) {
				$badStuff = array_intersect( array( 'PAGENAME', 'FULLPAGENAME', 'NAMESPACE' ), array_keys( $templates[NS_TEMPLATE] ) );
				if ( $badStuff !== array() ) {
					wfErrorLog( "Bad templates " . implode( ', ', $badStuff ) . " in page [[{$article->getTitle()->getPrefixedText()}]] key $parserOutputKey and timestamp $now generated by $hostname\n", 'udp://10.0.5.8:8420/bug31576' );
				}
			}
