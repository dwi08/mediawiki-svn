...
class LinksUpdate {
	...
	function getTemplateInsertions( $existing = array() ) {
		...
		foreach( $this->mTemplates as $ns => $dbkeys ) {
			...
			foreach ( $diffs as $dbk => $id ) {
				// HACK for investigating bug 31576 --Roan
				if ( in_array( $dbk, array( 'NAMESPACE', 'PAGENAME', 'SERVER', 'CONTENTLANGUAGE', 'PAGENAMEE' ) ) ) {
					$now = wfTimestampNow();
					$hostname = wfHostname();
					wfErrorLog( "Bad template $dbk in page [[{$this->mTitle->getPrefixedText()}]] timestamp $now generated by $hostname callers " . wfGetAllCallers( false ) . "\n", 'udp://10.0.5.8:8420/bug31576' );
					ob_start();
					debug_print_backtrace();
					$backtrace = ob_get_contents();
					ob_end_clean();
					$backtrace = substr( $backtrace, -49999 );
					wfErrorLog( $backtrace . "\n", 'udp://10.0.5.8:8420/bug31576bt' );
				}
				$arr[] = array(
					'tl_from'      => $this->mId,
					'tl_namespace' => $ns,
